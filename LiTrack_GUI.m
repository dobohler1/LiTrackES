...
% Last Modified by GUIDE v2.5 06-Feb-2017 06:20:12
...
if run_ok
    ...
    if handles.es
        ...
        low_lims = pars-10*delta;
        high_lims = pars+10*delta;
...
if get(handles.es_checkbox, 'Value') ==1 
    if ~isfield(handles,'tablename')
        f = figure('Position', [200 200 1100 550]);
        handles.tablename = f;
    else
        f=handles.tablename;
    end
...
%     for i=1:25
%         rowname{i} = ['Device' num2str(i)];
%     end

for i=1:15
    columnname{i} = ['opt' num2str(i)];
end
       
    columnformat = {'numeric', 'logical','numeric', 'numeric', 'logical','numeric','numeric', ...
        'logical','numeric','numeric', 'logical','numeric','numeric', 'logical','numeric',};
%     rowname = {'Device1' 'Device2'};
%     columnname = {'Attribute 1' 'Optimize'};
    columneditable =  [false true true false true true false ...
        true true false true true false true true];
%     columnformat = {'numeric', 'logical'};
 
    datStat0 = num2cell(zeros(25,1));
    datStat1 = num2cell(ones(25,1));
    %g= num2cell(randi(100,25,1));
    beamline=num2cell(handles.inp_struc.beamline);
 [rowname]=Litrack_deviceKey(cell2mat(beamline(:,1)));
    dat=cell(25,15);
    for j = 1:5
        for i =1:25
            if j == 1
                dat{i,j} = beamline{i,j+1} ;%
            else 
                k=2*j+j-2;
                dat{i,k} = beamline{i,j+1} ;%
            end
        end
    end
    for j = 2:3:15
    ...
    handles.table1=uitable(f, 'Data', dat,'Units','normalized','Position',...
        [0.1 0.1 0.9 0.9],'ColumnName', columnname, 'RowName', rowname,'ColumnFormat', columnformat,'ColumnEditable', columneditable);    
end
guidata(hObject, handles)
...
function iter_textbox_Callback(hObject, eventdata, handles)



% --- Executes during object creation, after setting all properties.
function iter_textbox_CreateFcn(hObject, eventdata, handles)
